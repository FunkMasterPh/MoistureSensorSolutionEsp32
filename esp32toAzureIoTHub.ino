...
const int dryValue = 4095; //analog reading value when dry
const int wetValue = 2635; //analog reading value when completely wet 
int moistureValue = 0; //analog reading value 
int moisturePercentCurrent = 0; //mapped value from dryValue - wetValue
int moisturePercentPrev = 0; //when percentage changes store new value
...
static void getTelemetryPayload(az_span payload, az_span* out_payload)
{
  ...
  moisturePercentCurrent = map(moistureValue, dryValue, wetValue, 0, 100);

  payload = az_span_copy(payload, AZ_SPAN_FROM_STR("{ \"moistureValue\": "));
  (void)az_span_u32toa(payload, moisturePercentCurrent, &payload);
